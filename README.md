...
[🌍 서비스 바로가기](https://danim.kr) &nbsp;·&nbsp; [🎨 디자인 시스템](#) &nbsp;·&nbsp; [🐛 버그 제보](../../issues) &nbsp;·&nbsp; [💬 피드백](../../discussions)
...
**🗺️ 지도 기반 여행 경로 시각화**
스팟마다 번호 매겨진 마커와 polyline으로 실제 동선을 한눈에 확인하세요.
게시글 전환 시 비행기가 날아가며 지도를 가리는 모션으로 raster 타일 로딩 깜빡임을 마스킹합니다.

<br/>

**📸 스팟 단위 여행 기록**
한 게시글이 여러 스팟으로 구성되고, 스팟마다 위치·사진·텍스트를 입력하세요.
Canvas 기반 클라이언트 압축으로 5MB 사진이 평균 380KB로 줄어듭니다.
드래그로 스팟 순서를 재배치할 수 있으며 데스크탑·모바일 모두 동작합니다.

<br/>

**🔗 모달과 페이지의 경계 — 인터셉트 라우트**
앱 내에서 게시글을 클릭하면 모달 오버레이로, 카카오톡 공유 링크로 들어오면 SSR 풀페이지로.
같은 URL이 진입 경로별로 다른 렌더링을 제공합니다 (Next.js Parallel + Intercepting Routes).

<br/>

**💬 실시간 DM**
좋은 식당을 발견했나요? DM으로 바로 공유하세요.
WebSocket 기반 1:1 실시간 메시지에 이미지 첨부, 옵티미스틱 업데이트 지원.

<br/>

**🔔 실시간 알림**
좋아요·댓글·팔로우·DM 알림이 사이드바 뱃지에 실시간으로 표시됩니다.
HttpOnly 쿠키로는 WebSocket 헤더 인증이 불가하므로 `socket_key` 발급 → WS URL query 인증 패턴을 직접 설계.

<br/>

**🖼️ 이미지 최적화**
next/image의 priority·sizes·lazy 자동화에 `browser-image-compression`(Web Worker + Canvas) 기반 클라이언트 압축을 결합해 트래픽을 92% 절감하고 LCP를 4.5s에서 2.1s로 단축했습니다. 1차 압축(1920px / quality 0.8)에서 10MB 초과 시 2차 압축(1280px / quality 0.6)으로 단계적 처리합니다.
