...
import type { SignupRequest, SignupResponse, TokenRefreshResponse, LoginRequest, LoginResponse, CurrentUserResponse, ConfirmEmailResponse, DetailResponse, SendSmsRequest, VerifySmsRequest, VerifySmsResponse } from '@/types'
...
// SMS 인증 코드 발송 — purpose에 따라 회원가입/이메일 찾기/휴대폰 변경 코드 구분
export async function sendSmsVerification(data: SendSmsRequest): Promise<DetailResponse> {
  return publicClient
    .post('users/verification/send-sms', { json: data })
    .json<DetailResponse>()
}

// SMS 인증 코드 검증 — 성공 시 백엔드가 sms_token 발급
export async function verifySmsCode(data: VerifySmsRequest): Promise<VerifySmsResponse> {
  return publicClient
    .post('users/verification/verify-sms', { json: data })
    .json<VerifySmsResponse>()
}
...
// 로그아웃 — refresh_token HttpOnly 쿠키를 서버에서 무효화
export async function logout(): Promise<DetailResponse> {
  return apiClient.post('users/logout').json<DetailResponse>()
}
