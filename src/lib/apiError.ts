...
// 에러에서 토스트에 표시할 문자열을 추출
// 우선순위: 백엔드 메시지 → fallback(client/server)
export function getApiErrorMessage(
  err: unknown,
  fallback: { client: string; server: string } // 백엔드 메시지가 없을 때 대신 쓸 문구
): string {
  if (isApiError(err)) {
    const { detail } = err
    ...
    // 백엔드 메시지를 못 꺼냈을 때 status 코드로 fallback 결정
