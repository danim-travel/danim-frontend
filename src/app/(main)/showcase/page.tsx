  ...
  return (
    <div className="h-full overflow-y-auto bg-bg">
      <div className="max-w-4xl mx-auto px-8 py-10">
        <div className="mb-10">
          <h1 className="text-2xl font-bold text-text">컴포넌트 쇼케이스</h1>
          <p className="text-sm text-text-disabled mt-1">공통 컴포넌트 목록 및 동작 확인</p>
          <div className="flex gap-2 mt-4">
            {SECTIONS.map((s) => (
              <a
                key={s.id}
                href={`#${s.id}`}
                className="px-3 py-1 rounded-full text-xs font-medium bg-bg-card border border-border text-text-body hover:border-primary hover:text-primary transition-colors"
              >
                {s.label}
              </a>
            ))}
        ...
        <section id="sidenav" className="mb-14">
          ...
          />
          <div className="bg-bg-card rounded-2xl border border-border-subtle p-6 text-sm text-text-disabled">
            현재 화면 왼쪽에 표시 중
          </div>
        </section>
        ...
        <section id="kakaomap" className="mb-14">
          ...
          />
          <div className="rounded-2xl overflow-hidden border border-border-subtle" style={{ height: 400 }}>
        ...
        <section id="postmodal" className="mb-14">
          ...
          <div className="bg-bg-card rounded-2xl border border-border-subtle p-6 flex items-center gap-4">
            ...
            <span className="text-xs text-text-disabled">postId: &quot;showcase-post&quot; (MSW mock)</span>
          </div>
        </section>
      </div>
  ...
  return (
    <div className="mb-4">
      <div className="flex items-baseline gap-3 mb-1">
        ...
        <code className="text-xs text-text-disabled font-mono">{path}</code>
      </div>
